...
echo ""
echo "Default MinIO credentials: accessKey=minioadmin secretKey=minioadmin"
echo ""
echo "In a Syncthing config.xml, set (prefer env credentials over URI secrets):"
echo "  <folder id=\"my-s3-folder\" label=\"S3 Backed Folder\""
echo "    path=\"s3://${ENDPOINT}/${TEST_BUCKET}\""
echo "    type=\"sendreceive\" rescanIntervalS=\"60\" fsWatcherEnabled=\"false\">"
echo "      <filesystemType>s3</filesystemType>"
echo "      <device id=\"YOUR-DEVICE-ID\" />"
echo "  </folder>"
echo ""
echo "  export S3_ACCESS_KEY_ID=${MINIO_ROOT_USER}"
echo "  export S3_SECRET_ACCESS_KEY=${MINIO_ROOT_PASSWORD}"
echo ""
